...
const seedQuestions = async () => {
  try {
    const existing = await docClient.send(new ScanCommand({ TableName: TABLES.QUESTIONS, Limit: 5 }));
    if (existing.Items && existing.Items.length >= 100) {
      console.log("✅ Question Bank is already full (100+ items)");
      return;
    }
    ...
    console.log("📊 SUCCESS: AWS Native Question Bank fully hydrated!");
  } catch (err) {
...
app.listen(PORT, () => { console.log(`🚀 AWS NATIVE SERVER RUNNING ON PORT ${PORT}`); });