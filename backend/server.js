...
mongoose.connect(MONGO_URI)
  .then(() => {
    ...
    // Count results to prove it's working
    Result.countDocuments().then(count => {
      console.log(`📊 TOTAL RESULTS IN CLOUD: ${count}`);
    });
  ...
  .catch(err => {
    console.log("❌ FAILED: Could not connect to Atlas:");
...
const ResultSchema = new mongoose.Schema({
  ...
  total: Number,
  createdAt: { type: Date, default: Date.now }
}, { collection: 'results' }); // Explicitly match Atlas 'results'
...
const seedUsers = async () => {
  ...
  for (const s of students) {
    ...
    if (!existing) {
      ...
      console.log(`Seeded student: ${s.username}`);
    }
  }
  for (const f of faculties) {
    const existing = await Faculty.findOne({ username: f.username });
    if (!existing) {
      const hashedPassword = await bcrypt.hash(f.password, 10);
      await Faculty.create({ ...f, password: hashedPassword });
      console.log(`Seeded faculty: ${f.username}`);
    }
  }
};
...
app.post("/register", async (req, res) => {
  try {
    ...
    if (role === 'student') {
      ...
      const faculty = new Faculty({ username, password: hashedPassword });
      await faculty.save();
    }
...
app.post("/login", async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: user._id, role: user.role, username: user.username }, process.env.JWT_SECRET || 'secret');
...
app.post("/result", async (req, res) => {
  try {
    console.log("📥 Receiving new result:", req.body);
    const result = new Result(req.body);
    await result.save();
    console.log("✅ Result saved to Atlas Cloud!");
